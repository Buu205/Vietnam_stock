...
class VNIndexValuationCalculator:
    ...
    def load_bsc_forecast_data(self):
        """
        Load BSC Forecast data from Excel.
        Returns: DataFrame [symbol, npat_2025, npat_2026]
        """
        path = self.base_path / 'DATA' / 'processed' / 'forecast' / 'Database Forecast BSC.xlsx'
        if not path.exists():
            logger.warning(f"⚠️ BSC Forecast file not found at {path}")
            return None
            
        try:
            # Read Sheet3, Transpose
            # Sheet3 format: Rows are metrics, Columns are tickers
            df = pd.read_excel(path, sheet_name='Sheet3', index_col=0) # Index is metric name
            df_t = df.T # Transpose: Index is now Ticker
            df_t.reset_index(inplace=True)
            df_t.rename(columns={'index': 'symbol'}, inplace=True)
            
            cols = ['symbol', '2025_npat', '2026_npat']
            # Ensure columns exist
            existing_cols = [c for c in cols if c in df_t.columns]
            if len(existing_cols) < 2:
                logger.warning("Forecast file missing required columns (2025_npat, 2026_npat)")
                return None
                
            forecast_df = df_t[existing_cols].copy()
            # Normalize
            forecast_df['symbol'] = forecast_df['symbol'].str.upper().str.strip()
            # Convert to numeric
            for c in ['2025_npat', '2026_npat']:
                if c in forecast_df.columns:
                    forecast_df[c] = pd.to_numeric(forecast_df[c], errors='coerce') * 1e9 # Convert Billions to Ones
